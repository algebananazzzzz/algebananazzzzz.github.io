...
const EDGES: [string, string][] = [
  ['yeast', 'saison'], ['yeast', 'hopping'], ['saison', 'hopping'],
  ['metaprompt', 'tool-design'], ['metaprompt', 'ctx-window'], ['metaprompt', 'eval'],
  ['tool-design', 'ctx-window'], ['ctx-window', 'eval'],
  ['iam', 'edge-cache'],
  ['garden', 'yeast'], ['garden', 'metaprompt'], ['garden', 'reading-21'],
  ['tool-design', 'iam'],
];
...
export default function Constellation({ notes, clusters, activeCluster = 'all' }: Props) {
  ...
  const positioned: PositionedNote[] = useMemo(() => notes.map((n) => {
  ...
  return (
    <div className="relative w-full">
      <svg viewBox={`0 0 ${VB_W} ${VB_H}`} className="w-full h-auto" style={{ aspectRatio: '200/60', maxHeight: 420 }} role="img" aria-label="Constellation of notes">
        <defs>
          ...
          {positioned.map((n) => {
            ...
            return (
              ...
              >
                ...
                {!lit && n.r > 1.0 && (
                  <circle
                    cx={n.cx} cy={n.cy} r={n.r * 3.0}
                    fill="url(#cons-haloGrad)"
                    opacity={isHover ? 0.95 : 0.6}
                  />
                )}
                <circle
                  cx={n.cx} cy={n.cy} r={rNode}
                  fill={c.dotColor}
