...
export default function MilkyWayStreak() {
  const dust = useMemo<Dust[]>(() => {
    ...
    for (let i = 0; i < 480; i++) {
      ...
      const r = Math.random() < 0.04 ? 0.6 + Math.random() * 0.8
              : Math.random() < 0.25 ? 0.3 + Math.random() * 0.4
              : 0.12 + Math.random() * 0.28;
      const op = 0.30 + Math.random() * 0.65;
  ...
  return (
    <svg viewBox="0 0 200 60" className="mw-streak" preserveAspectRatio="none" aria-hidden="true">
      ...
      <ellipse cx="100" cy="30" rx="110" ry="22" fill="url(#streakRadial)" opacity="1"
               transform="rotate(-8 100 30)" filter="url(#streakBlur)" />
      <ellipse cx="100" cy="30" rx="130" ry="12" fill="url(#streakRadial)" opacity="0.85"
               transform="rotate(-4 100 30)" filter="url(#streakBlur)" />
      <ellipse cx="110" cy="30" rx="80" ry="6" fill="#ffffff" opacity="0.18"
               transform="rotate(-7 110 30)" filter="url(#streakBlur)" />
      <ellipse cx="112" cy="28" rx="42" ry="11" fill="url(#streakWarm)" opacity="0.85"
               transform="rotate(-8 112 28)" filter="url(#streakBlur)" />
      <ellipse cx="30" cy="38" rx="30" ry="8" fill="url(#streakCool)" opacity="0.8"
               transform="rotate(-12 30 38)" filter="url(#streakBlur)" />
      <ellipse cx="170" cy="22" rx="26" ry="6" fill="url(#streakCool)" opacity="0.6"
               transform="rotate(-12 170 22)" filter="url(#streakBlur)" />

      <g filter="url(#streakBlur)" opacity="0.95">
        {dust.map((d, i) => (
          <circle key={i} cx={d.x} cy={d.y} r={d.r}
                  fill={d.warm ? '#FFE08A' : '#ffffff'} opacity={d.op} />
        ))}
      </g>
    </svg>
  );
